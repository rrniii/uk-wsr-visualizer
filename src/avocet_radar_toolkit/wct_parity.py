...
class WctParityResult:
    case_id: str
    avocet_status: str
    avocet_output: str | None
    avocet_sha256: str | None
    avocet_size: int | None
...
class WctParityReport:
    ...
    @property
    def ok(self) -> bool:
        base_ok = all(result.avocet_status == "complete" and result.wct_status in {"skipped", "complete"} for result in self.results)
        if not base_ok:
            return False
...
def wct_command(case: WctParityCase, config_path: Path, output_path: Path, wct_app: Path = DEFAULT_WCT_APP) -> list[str]:
    if case.request.format not in WCT_FORMAT_BY_AVOCET:
        raise ValueError(f"WCT parity is not configured for Avocet format {case.request.format!r}")
    input_path = case.wct_input_path or case.item.path
    return [
        str(wct_export_script(wct_app)),
        input_path,
        str(output_path),
        WCT_FORMAT_BY_AVOCET[case.request.format],
        str(config_path),
    ]
...
def _compare_geotiff(avocet_path: Path, wct_path: Path) -> dict[str, Any]:
    import rasterio  # type: ignore

    with rasterio.open(avocet_path) as avocet, rasterio.open(wct_path) as wct:
        avocet_values = avocet.read(1)
        wct_values = wct.read(1)
        avocet_bounds = list(avocet.bounds)
        wct_bounds = list(wct.bounds)
        return {
            "driver": "geotiff",
            "shape_match": avocet_values.shape == wct_values.shape,
            "avocet_shape": list(avocet_values.shape),
            "wct_shape": list(wct_values.shape),
            "crs_match": str(avocet.crs) == str(wct.crs),
            "avocet_crs": str(avocet.crs),
            "wct_crs": str(wct.crs),
            "bounds_match": _seq_close_enough(avocet_bounds, wct_bounds, tolerance=1.0e-3),
            "avocet_bounds": avocet_bounds,
            "wct_bounds": wct_bounds,
            "transform_match": _seq_close_enough(tuple(avocet.transform)[:6], tuple(wct.transform)[:6], tolerance=1.0e-6),
            "nodata_match": _close_enough(avocet.nodata, wct.nodata),
            "avocet_stats": _array_stats(avocet_values, avocet.nodata),
            "wct_stats": _array_stats(wct_values, wct.nodata),
            "array_difference": _array_difference(avocet_values, wct_values, avocet.nodata, wct.nodata),
        }
...
def _compare_cf_netcdf(avocet_path: Path, wct_path: Path) -> dict[str, Any]:
    import netCDF4  # type: ignore

    with netCDF4.Dataset(avocet_path) as avocet, netCDF4.Dataset(wct_path) as wct:
        avocet_var_name = _first_2d_numeric_variable(avocet)
        wct_var_name = _first_2d_numeric_variable(wct)
        payload: dict[str, Any] = {
            "driver": "cf_netcdf",
            "avocet_dimensions": {name: len(dim) for name, dim in avocet.dimensions.items()},
            "wct_dimensions": {name: len(dim) for name, dim in wct.dimensions.items()},
            "dimensions_match": {name: len(dim) for name, dim in avocet.dimensions.items()}
            == {name: len(dim) for name, dim in wct.dimensions.items()},
            "avocet_variable": avocet_var_name,
            "wct_variable": wct_var_name,
        }
        if avocet_var_name and wct_var_name:
            avocet_var = avocet.variables[avocet_var_name]
            wct_var = wct.variables[wct_var_name]
            avocet_values = avocet_var[:]
            wct_values = wct_var[:]
            avocet_nodata = getattr(avocet_var, "_FillValue", None)
            wct_nodata = getattr(wct_var, "_FillValue", None)
            payload.update(
                {
                    "variable_shape_match": avocet_values.shape == wct_values.shape,
                    "avocet_stats": _array_stats(avocet_values, avocet_nodata),
                    "wct_stats": _array_stats(wct_values, wct_nodata),
                    "array_difference": _array_difference(avocet_values, wct_values, avocet_nodata, wct_nodata),
                }
            )
        return payload


def _compare_shapefile(avocet_path: Path, wct_path: Path) -> dict[str, Any]:
    import shapefile  # type: ignore

    with shapefile.Reader(str(avocet_path)) as avocet, shapefile.Reader(str(wct_path)) as wct:
        avocet_fields = [field[0] for field in avocet.fields[1:]]
        wct_fields = [field[0] for field in wct.fields[1:]]
        return {
            "driver": "shapefile",
            "shape_count_match": len(avocet.shapes()) == len(wct.shapes()),
            "avocet_shape_count": len(avocet.shapes()),
            "wct_shape_count": len(wct.shapes()),
            "record_count_match": len(avocet.records()) == len(wct.records()),
            "avocet_record_count": len(avocet.records()),
            "wct_record_count": len(wct.records()),
            "fields_match": avocet_fields == wct_fields,
            "avocet_fields": avocet_fields,
            "wct_fields": wct_fields,
            "bbox_match": _seq_close_enough(list(avocet.bbox), list(wct.bbox), tolerance=1.0e-5),
            "avocet_bbox": list(avocet.bbox),
...
def _compare_kmz(avocet_path: Path, wct_path: Path) -> dict[str, Any]:
    with zipfile.ZipFile(avocet_path) as avocet_zip, zipfile.ZipFile(wct_path) as wct_zip:
        avocet_entries = sorted(avocet_zip.namelist())
        wct_entries = sorted(wct_zip.namelist())
    avocet_box = _latlon_box_from_kmz(avocet_path)
    wct_box = _latlon_box_from_kmz(wct_path)
    box_match = avocet_box is not None and wct_box is not None and all(
        _close_enough(avocet_box[name], wct_box[name], tolerance=1.0e-5) for name in ("north", "south", "east", "west")
    )
    return {
        "driver": "kmz",
        "avocet_entries": avocet_entries,
        "wct_entries": wct_entries,
        "avocet_latlon_box": avocet_box,
        "wct_latlon_box": wct_box,
        "latlon_box_match": box_match,
    }


def compare_outputs(avocet_path: Path, wct_path: Path, export_format: str) -> dict[str, Any]:
    avocet_sha, avocet_size = _file_record(avocet_path)
    wct_sha, wct_size = _file_record(wct_path)
    comparison: dict[str, Any] = {
        "format": export_format,
        "avocet_path": str(avocet_path),
        "wct_path": str(wct_path),
        "avocet_size": avocet_size,
        "wct_size": wct_size,
        "size_difference_bytes": None if avocet_size is None or wct_size is None else avocet_size - wct_size,
        "same_sha256": bool(avocet_sha and wct_sha and avocet_sha == wct_sha),
    }
    try:
        if export_format == "geotiff":
            comparison.update(_compare_geotiff(avocet_path, wct_path))
        elif export_format == "cf_netcdf":
            comparison.update(_compare_cf_netcdf(avocet_path, wct_path))
        elif export_format == "shapefile":
            comparison.update(_compare_shapefile(avocet_path, wct_path))
        elif export_format == "kmz":
            comparison.update(_compare_kmz(avocet_path, wct_path))
        else:
...
def run_parity_case(
    ...
    avocet_job = run_export(case.request, case.item, case_dir / "avocet")
    avocet_output = Path(avocet_job.output_path) if avocet_job.output_path else None
    avocet_sha, avocet_size = _file_record(avocet_output) if avocet_output else (None, None)
    ...
    comparable = bool(avocet_output and avocet_output.exists() and wct_output.exists())
    if comparable and avocet_size != wct_size:
        notes.append(f"output sizes differ: Avocet={avocet_size}, WCT={wct_size}")
    comparison: dict[str, Any] = {}
    parity_status = "not_comparable"
    if comparable and avocet_output is not None:
        comparison = compare_outputs(avocet_output, wct_output, case.request.format)
    ...
    return WctParityResult(
        ...
        avocet_status=avocet_job.status,
        avocet_output=str(avocet_output) if avocet_output else None,
        avocet_sha256=avocet_sha,
        avocet_size=avocet_size,
        wct_command=command,
