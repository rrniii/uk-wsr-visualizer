...
def public_landing_html(payload: dict[str, Any]) -> str:
    ...
    title = value(dataset, "title", "Avocet UK radar aggregate HDF5")
...
<body>
  ...
  <h2>Public Links</h2>
  <ul>{link_html}</ul>
  <p>This page is generated by the Avocet Radar Toolkit publication workflow.</p>
</body>
</html>
"""
...
def build_publication_plan(
    ...
    if config.publish_stac:
        ...
        for item in items:
            ...
            relative = Path("catalog") / "stac" / "avocet-uk-radar-aggregate-h5" / f"{item.item_id}.json"
            source = _write_generated_json(staging_dir, relative, stac_payload)
            plan.objects.append(
                _object_from_file(
                    "stac_item",
                    config.public_bucket,
                    stac_object_key("avocet-uk-radar-aggregate-h5", item.item_id, config.object_prefix),
                    source,
                    public_base_url,
                )
            )
