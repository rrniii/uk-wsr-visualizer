"""Catalog scanning for Avocet aggregate HDF5 files."""
...
def scan_raw_volume(path: Path, raw_volume_base: Path, object_store_base: str = "") -> tuple[str, str, str, RawVolumeRecord, list[QuantityRecord], dict[str, Any]]:
    ...
    if not match:
        raise ValueError(f"not an Avocet raw-volume filename: {path}")
    ...
    with h5py.File(path, "r") as h5:
        ...
        root_attrs["avocet:source_type"] = "raw_volume_day"
        root_attrs["avocet:spatial"] = _spatial_metadata_root_volume(h5)
...
def scan_aggregate(path: Path, aggregate_base: Path, object_store_base: str = "") -> CatalogItem:
    ...
    if not match:
        raise ValueError(f"not an Avocet aggregate filename: {path}")
    date = match.group("date")
    radar_num = match.group("num")
    ...
    with h5py.File(path, "r") as h5:
        ...
        if spatial:
            root_attrs["avocet:spatial"] = spatial
...
def scan_aggregate_fast(path: Path, aggregate_base: Path, object_store_base: str = "") -> CatalogItem:
    ...
    return CatalogItem(
        ...
        object_key=object_key,
        object_url=join_object_url(object_store_base, object_key),
        root_attrs={"avocet:catalog_mode": "fast"},
        quantities_by_pulse={},
        times_by_pulse={},
    )
...
def build_raw_volume_catalog(
    ...
    for path in iter_raw_volume_files(raw_volume_base, radar, year, date):
        ...
        if root_attrs.get("avocet:spatial") and "avocet:spatial" not in group["root_attrs"]:
            group["root_attrs"]["avocet:spatial"] = root_attrs["avocet:spatial"]
