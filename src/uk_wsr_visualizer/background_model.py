...
def apply_background_model(
    ...
    low_sqi_frequency_min = float(getattr(config, "background_low_sqi_frequency_min", 0.40))
    ...
    velocity_abs_max = float(getattr(config, "static_clutter_vrad_abs_max_ms", 1.0))
    sqi_medium = float(getattr(config, "sqi_medium", 0.45))
    rhohv_weak = float(getattr(config, "rhohv_weak", 0.75))
    zdr_min = float(getattr(config, "zdr_min_db", -3.0))
    zdr_max = float(getattr(config, "zdr_max_db", 8.0))
    ...
    score = np.zeros(gates.shape, dtype="int16")
    evidence_counts: dict[str, int] = {
        ...
        "learned_low_sqi": 0,
        "current_low_sqi": 0,
        "learned_dualpol_unstable": 0,
        "current_dualpol_unstable": 0,
    }
    ...
    velocity = _field(companions, VRAD_CANDIDATES)
    if velocity is not None:
        current_static = np.isfinite(velocity) & (np.abs(velocity) <= velocity_abs_max)
        score += current_static.astype("int16")
        evidence_counts["current_static_vrad"] = int((base & current_static).sum())

    learned_low_sqi = _array(model, "low_sqi_frequency") >= low_sqi_frequency_min
    score += learned_low_sqi.astype("int16")
    evidence_counts["learned_low_sqi"] = int((base & learned_low_sqi).sum())

    sqi = _field(companions, SQI_CANDIDATES)
    if sqi is not None:
        current_low_sqi = np.isfinite(sqi) & (sqi < sqi_medium)
        score += current_low_sqi.astype("int16")
        evidence_counts["current_low_sqi"] = int((base & current_low_sqi).sum())

    learned_dualpol = (
        (_array(model, "low_rhohv_frequency") >= low_sqi_frequency_min)
        | (_array(model, "unstable_rhohv_frequency") >= low_sqi_frequency_min)
        | (_array(model, "zdr_outlier_frequency") >= low_sqi_frequency_min)
        | (_array(model, "unstable_zdr_frequency") >= low_sqi_frequency_min)
    )
    score += learned_dualpol.astype("int16")
    evidence_counts["learned_dualpol_unstable"] = int((base & learned_dualpol).sum())

    current_dualpol = np.zeros(gates.shape, dtype=bool)
    rhohv = _field(companions, RHOHV_CANDIDATES)
    if rhohv is not None:
        rhohv_texture = local_texture_array(rhohv)
        current_dualpol |= np.isfinite(rhohv) & ((rhohv < rhohv_weak) | (np.isfinite(rhohv_texture) & (rhohv_texture >= 0.15)))
    zdr = _field(companions, ZDR_CANDIDATES)
    if zdr is not None:
        zdr_texture = local_texture_array(zdr)
        current_dualpol |= np.isfinite(zdr) & (
            (zdr < zdr_min) | (zdr > zdr_max) | (np.isfinite(zdr_texture) & (zdr_texture >= 2.0))
        )
    score += current_dualpol.astype("int16")
    evidence_counts["current_dualpol_unstable"] = int((base & current_dualpol).sum())
