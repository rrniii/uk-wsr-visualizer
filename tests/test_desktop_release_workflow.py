...
class DesktopReleaseWorkflowTests(unittest.TestCase):
    def test_desktop_beta_smoke_script_documents_release_gate(self):
        ...
        self.assertIn("build/xcode-macos/UK WSR Visualizer.app", script)
        self.assertIn("Packaged Mac app is missing Contents/Resources/repo", script)
        self.assertIn("macos/build-xcode-macos.sh", script)
