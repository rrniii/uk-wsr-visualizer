...
TOOLKIT_DIR="${TOOLKIT_DIR:-$HOME/avocet-radar-toolkit}"
...
DATA_DIR="${DATA_DIR:-data/avocet-wct}"
PUBLIC_BASE="${PUBLIC_BASE:-https://ncas-radar-o.s3-ext.jc.rl.ac.uk/avocet-uk-radar-public}"
MONTHS="${MONTHS:-}"
DATES="${DATES:-}"
...
"$PYTHON_BIN" -B -m avocet_radar_toolkit.cli catalog build \
...
for batch_catalog in "$backfill_dir"/catalog-"$YEAR"????.json; do
  ...
  "$PYTHON_BIN" -B -m avocet_radar_toolkit.cli object-store plan \
    --config "$CONFIG" \
...
import sys
from pathlib import Path
from avocet_radar_toolkit.object_store_manifest import load_plan, write_plan
...
PY
  ...
  "$PYTHON_BIN" -B -m avocet_radar_toolkit.cli object-store sync \
    --execute \
    --skip-existing \
    --config "$CONFIG" \
    --plan "$raw_plan" \
  ...
  "$PYTHON_BIN" -B -m avocet_radar_toolkit.cli object-store verify \
    --execute \
    --config "$CONFIG" \
    --manifest "$synced" \
    --output "$verified"
...
"$PYTHON_BIN" -B -m avocet_radar_toolkit.cli object-store publish \
  --execute \
  --config "$CONFIG" \
