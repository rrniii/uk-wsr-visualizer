...
def create_s3_client(config: ObjectStoreConfig, internal: bool = True) -> Any:
    ...
    client._avocet_transfer_config = TransferConfig(multipart_chunksize=config.multipart_chunk_mb * 1024 * 1024)
    return client
...
def sync_plan(
    ...
    for obj in plan.objects:
        ...
        extra_args = {
            ...
            "Metadata": {
                "sha256": obj.sha256,
                "avocet-kind": obj.kind,
                "avocet-run-id": plan.run_id,
            },
        }
        assert client is not None
        transfer_config = getattr(client, "_avocet_transfer_config", None)
