...
| Optional mobile-copy output | pass `--ios-dir` or another external output directory when training models for a separate mobile checkout |
| Validation report | generated by `tools/train_learned_background_models.py --report-root ...`; keep large generated reports outside the source tree |
...
The model loader rejects mismatched model keys before application, so no
artifact is used on another radar by shape alone.
...
default app/export cleanup path in the desktop apps for matching scans.
...
3. Wire persisted `qc_mask` artifacts into VP batch runs and publication
   manifests.
4. Produce archive-scale summary reports: companion-field coverage, per-flag mask
   rates, decode failures, runtime, and cache footprint by radar/day/pulse.
5. Add manual review plots for the highest-risk cases: static clutter, dual-pol
