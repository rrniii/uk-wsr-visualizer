...
the release evidence archive; regenerate it locally with the documented audit
tools when reviewing a new model.
...
The immutable source selection is retained with the release evidence archive,
alongside its checksums and the command used to build it.
...
integrity audit is stored with the release evidence archive; a portable
...
in the release evidence archive.
...
the release evidence archive. It contains:
