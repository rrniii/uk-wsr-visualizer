...
- `src/uk_wsr_visualizer/preview.py`: generates browser-friendly PNG previews from selected ODIM quantity groups.
- `src/uk_wsr_visualizer/tiles.py`: generates preview-derived XYZ-style tile pyramids and tile manifests for browser/object-store delivery.
...
- `src/uk_wsr_visualizer/export.py`: records export jobs and supports native HDF5, metadata JSON, PNG preview, georeferenced KMZ, field CSV, GeoTIFF, CF NetCDF, GeoJSON contours, Shapefile contours, WCT-style batch config XML exports, artifact manifests, and safe download bundles.
- `src/uk_wsr_visualizer/math_ops.py`: creates WCT-style derived products from two radar fields using difference, sum, product, ratio, mean, min, or max operations.
...
- `src/uk_wsr_visualizer/freshness.py`: checks catalog age, data latency, object-store manifest verification, public inventory private-path redaction, and required WCT validation report parity status.
- `src/uk_wsr_visualizer/wct_parity.py`: generates WCT 4.9.1 batch configs, runs UK WSR Visualizer exports, optionally runs WCT batch export, and writes parity validation reports with strict comparison metrics for GeoTIFF, CF NetCDF, Shapefile, and KMZ.
- `src/uk_wsr_visualizer/api/app.py`: FastAPI app serving API endpoints and the static web UI.
- `src/uk_wsr_visualizer/session.py`: stores and loads WCT-style viewer sessions as JSON and imports/exports portable `uk-wsr-visualizer-project` files.
- `src/uk_wsr_visualizer/static/`: WCT-style browser UI with catalog search, radar controls, range/azimuth/value filters, palette/opacity/basemap controls, four-panel view, animation, tile-product generation, data-derived contour overlays, snapshots, click-to-identify readout, session/project state, and export actions.
...
- Public release readiness can be checked with `uk-wsr-visualizer freshness check --require-object-store --require-wct-validation`, `/api/freshness?require_object_store=true&require_wct_validation=true`, or the full `uk-wsr-visualizer object-store release-candidate` summary workflow.
