...
All object-store operations are dry-run by default. The systemd publisher uses `--execute`, so only enable `uk-wsr-visualizer-object-store-publish.timer` after the JASMIN tenancy, buckets, credentials, CORS, public redistribution permission, and required WCT validation reports are confirmed.
...
For WCT-style export parity checks, verify `geotiff`, `cf_netcdf`, `geojson`, `shapefile`, and georeferenced `kmz` outputs against representative UK WSR aggregate files and WCT 4.9.1 output.
