...
UK WSR Visualizer now has an Xcode-managed macOS shell:
...
The current `master` branch ships the Xcode-managed macOS shell, not a native
iOS target. Simulator and TestFlight remain useful for a later mobile target,
but that should be integrated deliberately as a separate app target with its own
data-cache, catalogue, and HDF5-decoding tests.
...
Python server are resolved. If TestFlight is used for macOS later, keep the same
archive scheme and confirm the bundled server/runtime passes App Store Connect
validation.
